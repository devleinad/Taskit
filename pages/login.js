...
function Login({csrfToken}) {
    ...
    if(!isEmpty(email) && !isEmpty(password)){
        loginButton =  <button type='submit' className='p-2 w-full bg-blue-400 text-white font-semibold text-lg rounded hover:bg-blue-600'>Login</button>
    }else{
        loginButton =  <button type='submit' className='p-2 w-full bg-slate-100 text-slate-600 font-semibold text-lg rounded hover:bg-blue-600' disabled>Login</button>
    }
                ...
                <div className='p-2'>
                    <form onSubmit={handleLoginSubmit}>
                     ...
                     <div className='mt-1 text-center'>
                        <div className='text-sm'>Don&apos;t have an account? <Link href={'/signup'}><span className='cursor-pointer text-blue-500'>Signup</span></Link></div>
                    </div>
