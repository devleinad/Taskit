    ...
    <div>
      <Head>
        ...
        <Layout user={user && user}>
                <h1 className='text-2xl md:text-3xl tracking-tight'>Welcome, <span className='font-semibold'>{user?.name}</span></h1>
                 <p className='text-gray-500 text-sm font-semibold'>All your projects and tasks are intact.</p>
                    ...
                    <Link key={data.title} href={data.link}>
                     ...
                     <div>
                       ...
                       <p className='mt-1 text-gray-400 font-semibold text-md text-center px-2 py-1 bg-slate-100 rounded-full'>
                         {data.count > 100 ? '100' : data.count}{data.count > 100 && <span className='font-bold'>+</span>}
                        </p>
                     </div>
                    </div>
                    </Link>
...
export const getServerSideProps = async (context) => {
    ...
    if(!session){
        return {
            redirect : {
                destination : 'https://taskit-ofcbvfhij-devleinad.vercel.app/login',
                permanent:false,
            }
        }
    }else{
